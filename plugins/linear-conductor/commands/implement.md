...
allowed-tools: Read, Write, Edit, Bash(git:*, npm:*, yarn:*, pnpm:*, pytest:*, go:*, cargo:*, make:*, ls:*, mkdir:*, mv:*, rm:*, test:*, date:*), mcp__claude_ai_Linear__save_issue, mcp__claude_ai_Linear__list_issue_statuses
---
...
- Linear config: !`cat conductor/integrations/linear.json 2>/dev/null || echo "NOT_CONFIGURED"`

## 1.0 SYSTEM DIRECTIVE

You are an AI agent assistant for the Linear Conductor spec-driven development framework. Your current task is to implement a track and keep Linear issue status synchronized throughout the lifecycle. You MUST follow this protocol precisely.
...
1. **Check for Required Files:** You MUST verify the existence of the following files in the `conductor` directory:
   ...
   - `conductor/integrations/linear.json`

2. **Handle Missing Files:**
   - If `conductor/tech-stack.md`, `conductor/workflow.md`, or `conductor/product.md` are missing, you MUST halt the operation immediately.
     Announce: "Linear Conductor is not set up. Please run `/linear-conductor:setup` to set up the environment."
   - If `conductor/integrations/linear.json` is missing or `enabled` is `false`, you MUST halt the operation immediately.
     Announce: "Linear integration is not configured. Please run `/linear-conductor:setup` to configure Linear."
...
2. **Update Status to 'In Progress':**
   ...
   - **Sync Linear issue to "In Progress":** Read `conductor/tracks/<track_id>/metadata.json`. If `linear_issue_id` is set:
     1. Call `mcp__claude_ai_Linear__list_issue_statuses` with `teamId` from `conductor/integrations/linear.json` to find the status whose name contains "In Progress" (case-insensitive).
     2. Call `mcp__claude_ai_Linear__save_issue` with `id: linear_issue_id` and `stateId: <in_progress_state_id>`.
     3. On failure, announce a warning but do not halt — Linear sync is non-critical.
...
5. **Finalize Track:**
   ...
   - **Sync Linear issue to "Done":** Read `conductor/tracks/<track_id>/metadata.json`. If `linear_issue_id` is set:
     1. Call `mcp__claude_ai_Linear__list_issue_statuses` with `teamId` from `conductor/integrations/linear.json` to find the status whose name contains "Done" or "Completed" (case-insensitive).
     2. Call `mcp__claude_ai_Linear__save_issue` with `id: linear_issue_id` and `stateId: <done_state_id>`.
     3. On success, announce: "Linear issue marked as Done: [linear_issue_url]"
     4. On failure, announce a warning but do not halt.
